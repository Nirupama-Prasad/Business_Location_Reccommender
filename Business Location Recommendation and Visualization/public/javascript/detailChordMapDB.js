...
function displayChordMap1(matrix1)
{
	//console.log(matrix1[3]);
	//alert(matrix1);
	//var matrix1 = [[matrix1[0],matrix1[1],matrix1[2],matrix1[3]],[matrix1[4],matrix1[5],matrix1[6],matrix1[7]],[matrix1[8],matrix1[9],matrix1[10],matrix1[11]],[matrix1[12],matrix1[13],matrix1[14],matrix1[15]]];
...
function updateChords( datasetURL ) {
  ...
  //d3.json("datajson1.json", function(error, matrix) {
	//alert("in");
	  //$.get( "../javascript/datajson1.json", function(data) 
//d3.json("../javascript/datajson1.json", function(error, data)
	// {
 
	 var data = JSON.parse( d3.select(datasetURL).text() )
 // alert("out");
	//alert(datasetURL);
	//alert("check");
	//alert(d3.select(datasetURL).toString());
	//matrix= JSON.parse(d3.select(datasetURL).toString());
    //alert("after csv"+data);
        // instead of d3.json
    ...
    newGroups = groupG.enter().append("g")
        .attr("class", "group");
    ...
    chordPaths.select("title")
        .text(function(d) {
            if (neighborhoods[d.target.index].name !== neighborhoods[d.source.index].name) {
                ...
                return numberWithCommas(d.source.value) 
                    + " people go to ",domain ," within this area " 
                    + neighborhoods[d.source.index].name;
            }
        });
